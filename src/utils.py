...
file_handler = logging.FileHandler(os.path.join(os.path.dirname(os.path.dirname(__file__)), 'logs', 'utils.log'),
                                   'w', encoding='utf-8')
...
def get_financial_transaction_data(path: str) -> list[dict]:
    ...
    try:
        ...
        with open(path, 'r', encoding='utf-8') as transaction_file:
            logger.info(f'Преобразуем данные из файла в формате JSON в Python объект.')
            transaction_data = json.load(transaction_file)
            logger.info(f'Проверяем является ли Python объект списком.')
            if isinstance(transaction_data, list):
                logger.info(f'Python объект список. Возвращаем {transaction_data}')
                return transaction_data
            else:
                logger.info(f'Python объект не список. Возвращаем пустой список')
                return []
